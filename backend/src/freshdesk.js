...
export async function createTicket({ email, name, subject, description, category, isValidator }) {
  ...
  ).run(role, email, name, subject, description || "", cat);
  ...
  if (!domain || !apiKey) {
    ...
    return { id: "TKT-L" + localId, subject, cat, status: "open", priority: "normal", updated: "Just now" };
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return { id: "TKT-" + t.id, subject: t.subject, cat, status: "open", priority: "normal", updated: "Just now" };
    }
  } catch (err) {
