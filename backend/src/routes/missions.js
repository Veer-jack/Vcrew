...
router.post("/generate-tasks", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const prompt = `You are a Principal QA Engineer with 15+ years of experience. You think like a seasoned tester who goes beyond surface-level clicks - you look for edge cases, data integrity issues, UX dead-ends, and moments where the product breaks trust with the user.
...
Generate 5-7 PROFESSIONAL test cases that a seasoned QA engineer would run. NOT beginner tasks like "open the app and look around". These are structured, methodical scenarios that probe the product deeply.

MINDSET: You are a power user being paid for finding real issues. Go deep. Be specific. Challenge assumptions.

RULES:
1. Title must name the SPECIFIC flow or feature - never "Core flow" or "User experience"
2. Steps must be precise and include adversarial actions (wrong inputs, leave fields blank, go back mid-flow)
3. Include steps that test failure and error states
4. At least one task must test data persistence (close app, reopen - is data still there?)
5. At least one task must test an error or edge case (invalid input, empty state, boundary condition)
6. Final task must capture NPS and what would stop them from using the product again
7. Questions must extract expert-level observations - not "was it easy" but "at which step did friction occur and why"

QUESTION QUALITY BAR:
BAD: "Was the signup easy?" 
GOOD: "At which exact step did you first feel friction and what caused it?"
BAD: "Did you like the design?"
GOOD: "Which UI element felt most inconsistent with what you expected from this type of product?"
  ...
  "tasks": [
    {
      ...
      "severity": "crit",
      "steps": [
        "Precise action with exact detail",
        "Adversarial step - try entering invalid/unexpected data and observe the response",
        "Data integrity check - verify the result is correct and persists"
      ],
      "questions": [
        { "id": "q1", "text": "Expert diagnostic question about this specific feature", "type": "rating", "scale": 5 },
        { "id": "q2", "text": "Question with product-specific options", "type": "multiple_choice", "options": ["Specific option A", "Specific option B", "Specific option C"] },
        { "id": "q3", "text": "Did [specific thing] work correctly? Describe exactly what happened.", "type": "yes_no_detail" },
        { "id": "q4", "text": "What would a professional user expect here that this product does not provide?", "type": "text" }
      ],
      "proof": "screenshot",
      "min_time_seconds": 420
    }
  ]
}

severity: crit=core must work, imp=important UX/data, nice=enhancement
types: rating(scale:5), multiple_choice(options array), yes_no_detail, text
