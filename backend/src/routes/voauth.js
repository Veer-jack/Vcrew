...
router.get("/:provider/callback", async (req, res) => {
  ...
  try {
    ...
    if (!validator) {
      ...
      if (validator) {
        await db.prepare(`UPDATE validators SET oauth_provider = ?, oauth_id = ? WHERE id = ?`).run(key, profile.id, validator.id);
      }
    }
    ...
    if (!validator) {
      if (mode === "signin") {
        return res.redirect(`${FRONTEND}/validator/login?error=${encodeURIComponent("Account not found. Please create an account first.")}`);
      }
