...
function serializeThread(t, withMessages) {
  ...
  const msgs = db.prepare(`SELECT * FROM v_thread_messages WHERE thread_id = ? ORDER BY id ASC`).all(t.id);
  if (withMessages) {
...
router.get("/threads", (req, res) => {
  const threads = db.prepare(`SELECT * FROM v_threads WHERE validator_id = ?`).all(req.validator.id);
  res.json({ threads: threads.map(t => serializeThread(t, false)) });
});

router.get("/threads/:id", (req, res) => {
  const t = db.prepare(`SELECT * FROM v_threads WHERE id = ? AND validator_id = ?`).get(req.params.id, req.validator.id);
...
router.post("/threads/:id/messages", (req, res) => {
  ...
  db.prepare(`INSERT INTO v_thread_messages (thread_id, sender, text, time_label) VALUES (?, 'me', ?, 'Now')`).run(t.id, text);
  db.prepare(`UPDATE v_threads SET time_label = 'Now' WHERE id = ?`).run(t.id);
