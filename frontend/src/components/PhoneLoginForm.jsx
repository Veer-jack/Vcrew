...
export default function PhoneLoginForm({ client, userKey, onSuccess }) {
  ...
  const sendCode = async (e) => {
    ...
    try {
      ...
      if (!auth) throw new Error(t("phoneLogin.notConfigured", null, "Phone login isn't configured on this server yet"));
      ...
      setError(err.message || t("phoneLogin.couldntSendCode", null, "Couldn't send code"));
  ...
  const verify = async (e) => {
    ...
    try {
      ...
      setError(err.message || t("phoneLogin.couldntVerifyCode", null, "Couldn't verify code"));
    } finally { setBusy(false); }
  };
  ...
  return (
    <form onSubmit={step === "phone" ? sendCode : verify} className="col gap-4">
      ...
      {step === "phone" ? (
        <div className="fld">
          <label>{t("phoneLogin.phoneNumber", null, "Phone number")}</label>
          <input className="fin" type="tel" placeholder="+14155551234" value={phone} onChange={e => setPhone(e.target.value)} required autoFocus />
        </div>
      ) : (
        <div className="fld">
          <label>{t("phoneLogin.enterCodeSentTo", { phone }, `Enter the 6-digit code sent to ${phone}`)}</label>
      ...
      <Btn type="submit" variant="primary" size="lg" block disabled={busy}>
        {busy ? t("phoneLogin.pleaseWait", null, "Please wait…") : step === "phone" ? t("phoneLogin.sendCode", null, "Send code") : t("phoneLogin.verifyAndSignIn", null, "Verify and sign in")}
      </Btn>
      {step === "code" && (
        <button type="button" className="backlink" style={{ margin: "0 auto" }} onClick={() => { setStep("phone"); setCode(""); setError(""); }}>
          {t("phoneLogin.useDifferentNumber", null, "Use a different number")}
        </button>
      )}
    </form>
  );
