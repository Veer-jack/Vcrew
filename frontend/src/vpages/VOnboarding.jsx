...
function ValidatorOnboarding({ onDone }) {
  ...
  return (
    <div className="rise" style={{ maxWidth: 600, margin: "0 auto" }}>
      ...
      {step === 1 && (<><h2 style={{ fontSize: 22, fontWeight: 800, margin: "0 0 22px" }}>Professional background</h2><Field label="Role" required><Chips options={ROLES} value={d.role} onChange={v => set("role", v)} multi={false} /></Field><Field label="Experience" required><Chips options={EXP} value={d.experience} onChange={v => set("experience", v)} multi={false} /></Field><Field label="Industry" required><Chips options={INDUSTRIES} value={d.industry} onChange={v => set("industry", v)} /></Field><Field label="Company" hint="Optional"><input className="fin" value={d.company} onChange={e => set("company", e.target.value)} placeholder="e.g. Razorpay, Freelance" /></Field></>)}
      ...
      <div style={{ display: "flex", gap: 12, marginTop: 28 }}>
        {step > 0 && <button className="btn btn-ghost" onClick={() => setStep(s => s-1)} style={{ flex: 1 }}><Icon name="arrowLeft" size={15} /> Back</button>}
        <Btn variant="primary" style={{ flex: 2, justifyContent: "center" }} disabled={!valid[step]} onClick={() => step < 3 ? setStep(s => s+1) : onDone(d, "validator")}>{step === 3 ? "Complete setup" : "Continue"}</Btn>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", display: "grid", gridTemplateColumns: "260px 1fr" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 24px", overflowY: "auto" }}>
        ...
        {showPending ? <PendingScreen onContinue={() => navigate("/validator", { replace: true })} /> : !validatorType ? <TypeSelector onSelect={setValidatorType} /> : validatorType === "user" ? <UserOnboarding onDone={handleDone} /> : validatorType === "validator" ? <ValidatorOnboarding onDone={handleDone} /> : <TesterOnboarding onDone={handleDone} />}
      </div>
    </div>
  );
}
