...
// Mirrors CreateMissionWizard's FilterGroup "Add other" UX (dashed chip
// trigger -> inline input+Save/Cancel -> removable chip, multiple entries
// allowed) for a multi-select Chips field that includes a literal "Other"
// option -- simplified to a plain string array instead of FilterGroup's
// Set+title-keyed toggle API, which onboarding's chip fields don't use.
function OtherEntries({ values, onChange, placeholder }) {
  const { t } = useTranslation();
  ...
  const save = () => {
    const v = draft.trim();
    if (!v) return;
    if (list.some(e => e.toLowerCase() === v.toLowerCase())) { setDupError(true); return; }
    onChange([...list, v]);
    setDraft("");
    setAdding(false);
    setDupError(false);
  };
  const cancel = () => { setDraft(""); setAdding(false); setDupError(false); };
  return (
    <div style={{ marginTop: 8 }}>
      {list.length > 0 && (
        <div className="chips" style={{ marginBottom: 8 }}>
          {list.map((val, i) => (
            <div key={i} className="chip on" style={{ paddingRight: 6, gap: 0 }}>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}><span className="ck"><Icon name="check" size={10} /></span>{val}</span>
              <button type="button" onClick={() => onChange(list.filter((_, j) => j !== i))} style={{ marginLeft: 8, background: "none", border: 0, padding: 0, display: "inline-flex", cursor: "pointer", color: "inherit", opacity: 0.7 }}>
                <Icon name="x" size={11} />
              </button>
            </div>
          ))}
      ...
      {adding ? (
        <div>
          <div className="row gap-2" style={{ alignItems: "center" }}>
            <input className="fin" style={{ fontSize: 13, maxWidth: 220 }} autoFocus
              placeholder={placeholder || t("onboardingFields.customEntryPlaceholder", null, "Type and press Enter")}
              value={draft} onChange={e => { setDraft(e.target.value); setDupError(false); }}
              onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); save(); } if (e.key === "Escape") cancel(); }} />
            <button type="button" className="btn btn-primary" style={{ padding: "7px 14px", fontSize: 13 }} disabled={!draft.trim()} onClick={save}>{t("actions.save", null, "Save")}</button>
            <button type="button" className="btn outline" style={{ padding: "7px 14px", fontSize: 13 }} onClick={cancel}>{t("actions.cancel", null, "Cancel")}</button>
          </div>
          {dupError && <p className="ferr" style={{ marginTop: 6 }}>{t("createMission.otherDuplicateValue", null, "That value is already added.")}</p>}
        </div>
      ) : (
        <button type="button" className="chip" style={{ borderStyle: "dashed" }} onClick={() => setAdding(true)}>
          <Icon name="plus" size={12} style={{ marginRight: 4 }} />{t(list.length ? "createMission.addAnotherOther" : "createMission.addOtherTrigger", null, list.length ? "Add more" : "Add other")}
        </button>
      )}
    </div>
  );
...
function TesterOnboarding({ step, onNext, error, vid }) {
  ...
  return (
    <div className="rise" style={{ maxWidth: 600, margin: "0 auto" }}>
      {step === 0 && (<><h2 style={{ fontSize: 22, fontWeight: 800, margin: "0 0 8px" }}>{t("vOnboarding.headers.tellUsAboutYourself", null, "Tell us about yourself")}</h2><div style={{ padding: "10px 14px", background: "var(--warning-weak)", borderRadius: "var(--radius-sm)", fontSize: 13, color: "var(--warning)", fontWeight: 600, marginBottom: 22 }}>{t("vOnboarding.body.adminReviewsProfile72h", null, "Admin reviews your profile within 72 hours.")}</div><Field label={t("vOnboarding.fields.fullName", null, "Full name")} required><input className="fin" value={d.name} onChange={e => set("name", e.target.value)} placeholder={t("vOnboarding.placeholders.yourFullName", null, "Your full name")} /></Field><Field label={t("vOnboarding.fields.handle", null, "Handle")} required hint={t("vOnboarding.hints.lowercaseNoSpaces", null, "Lowercase, no spaces")} info={t("vOnboarding.hints.handleInfo", null, "Your unique @username on ValidationCrew — shown to builders on your profile and submissions.")}><div className="inw has-pre"><span className="pre">@</span><input className="fin" value={d.handle} onChange={e => set("handle", e.target.value.toLowerCase().replace(/\s/g,""))} placeholder={t("vOnboarding.placeholders.yourHandle", null, "yourhandle")} /></div></Field><Field label={t("vOnboarding.fields.city", null, "City")} required><input className="fin" value={d.city} onChange={e => set("city", e.target.value)} placeholder={t("vOnboarding.placeholders.cityBengaluruHyderabad", null, "e.g. Bengaluru, Hyderabad")} /></Field><CountryStateFields d={d} set={set} /><Field label={t("vOnboarding.fields.languages", null, "Languages")} action={<SelectAllToggle options={LANGUAGES} value={d.language} onChange={v => set("language", v)} />}><Chips options={LANGUAGES} value={d.language} onChange={v => set("language", v)} getLabel={optLabel(t, "languages")} />{(d.language.includes("Other") || (d.languageOther || []).length > 0) && <OtherEntries values={d.languageOther} onChange={v => set("languageOther", v)} />}</Field></>)}
      {step === 1 && (<><h2 style={{ fontSize: 22, fontWeight: 800, margin: "0 0 22px" }}>{t("vOnboarding.headers.professionalBackground", null, "Professional background")}</h2><Field label={t("vOnboarding.fields.role", null, "Role")} required><Chips options={ROLES} value={d.occupation || d.role} onChange={v => set("occupation", v)} multi={false} getLabel={optLabel(t, "roles")} /></Field>{(d.occupation || d.role) === "Other" && <Field label={t("onboardingFields.customRole", null, "Your role")}><input className="fin" value={d.occupationOther || ""} onChange={e => set("occupationOther", e.target.value)} placeholder={t("onboardingFields.customRolePlaceholder", null, "Type your role")} /></Field>}<Field label={t("vOnboarding.fields.experience", null, "Experience")} required><Chips options={EXP} value={d.experience} onChange={v => set("experience", v)} multi={false} getLabel={optLabel(t, "experience")} /></Field><Field label={t("vOnboarding.fields.currentCompany", null, "Current company")} required hint={t("vOnboarding.hints.companyRequiredVerification", null, "Required for verification")}><input className="fin" value={d.company} onChange={e => set("company", e.target.value)} placeholder={t("vOnboarding.placeholders.companyExampleTester", null, "e.g. Infosys, Freelance QA")} /></Field><Field label={t("vOnboarding.fields.industry", null, "Industry")} required action={<SelectAllToggle options={INDUSTRIES} value={d.industry} onChange={v => set("industry", v)} />}><Chips options={INDUSTRIES} value={d.industry} onChange={v => set("industry", v)} getLabel={optLabel(t, "industries")} />{(d.industry.includes("Other") || (d.industryOther || []).length > 0) && <OtherEntries values={d.industryOther} onChange={v => set("industryOther", v)} />}</Field><Field label={t("vOnboarding.fields.testingDomains", null, "Testing domains")} action={<SelectAllToggle options={TESTER_DOMAINS} value={d.domains} onChange={v => set("domains", v)} />}><Chips options={TESTER_DOMAINS} value={d.domains} onChange={v => set("domains", v)} getLabel={optLabel(t, "testerDomains")} />{(d.domains.includes("Other") || (d.domainsOther || []).length > 0) && <OtherEntries values={d.domainsOther} onChange={v => set("domainsOther", v)} />}</Field><Field label={t("vOnboarding.fields.certifications", null, "Certifications")} action={<SelectAllToggle options={CERT} value={d.certifications} onChange={v => set("certifications", v)} />}><Chips options={CERT} value={d.certifications} onChange={v => set("certifications", v)} getLabel={optLabel(t, "certifications")} />{(d.certifications.includes("Other") || (d.certificationsOther || []).length > 0) && <OtherEntries values={d.certificationsOther} onChange={v => set("certificationsOther", v)} />}</Field><Field label={t("vOnboarding.fields.tools", null, "Tools")} action={<SelectAllToggle options={TECH_TOOLS} value={d.tools} onChange={v => set("tools", v)} />}><Chips options={TECH_TOOLS} value={d.tools} onChange={v => set("tools", v)} getLabel={optLabel(t, "techTools")} /></Field></>)}
...
export default function VOnboarding() {
  ...
  const goNext = (data) => {
    if (step < totalSteps - 1) {
      ...
      // Always folds in whatever custom entries were saved, regardless of
      // whether the "Other" chip itself is still checked -- Select all /
      // Clear all only ever touch the plain option array (see
      // SelectAllToggle), so a Clear all after typing a custom entry must
      // not be able to silently drop it from what actually gets submitted.
      const resolveMulti = (arr, otherArr) => [...(arr || []).filter(v => v !== "Other"), ...((otherArr || []).filter(Boolean))];
      ...
      if ("language" in finalData) patch.language = resolveMulti(finalData.language, finalData.languageOther);
      if ("industry" in finalData) patch.industry = resolveMulti(finalData.industry, finalData.industryOther);
      if ("domains" in finalData) patch.domains = resolveMulti(finalData.domains, finalData.domainsOther);
      if ("certifications" in finalData) patch.certifications = resolveMulti(finalData.certifications, finalData.certificationsOther);
