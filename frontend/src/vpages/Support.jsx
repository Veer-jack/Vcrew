...
function RaiseTicket({ onClose, onCreated }) {
  ...
  const submit = async () => {
    ...
    try {
      const { ticket } = await vapi.raiseTicket({ category: cat, subject, details });
      setSent(ticket);
      onCreated(ticket);
    } catch (err) {
      setError(err.message || t("vSupport.couldntRaiseTicket", null, "Couldn't raise ticket"));
      ...
      <div style={{ position: "fixed", top: "50%", left: "calc(50% + 120px)", transform: "translate(-50%,-50%)", zIndex: 61 }}>
        ...
        {sent ? (
          <div style={{ padding: 40, textAlign: "center" }}>
            ...
            <p className="muted" style={{ margin: "0 0 20px", fontSize: 14 }}>{t("support.ticketLogged", { id: sent.id }, `We've logged your request as ${sent.id}. Expect a reply within a few hours.`)}</p>
          ...
          <>
            <div className="row between" style={{ padding: "18px 22px", borderBottom: "var(--hairline) solid var(--border)" }}>
              ...
              <button className="icon-btn" aria-label={t("vSupport.close", null, "Close")} style={{ width: 32, height: 32 }} onClick={onClose}><Icon name="x" size={16} /></button>
            </div>
            <div style={{ padding: 22, display: "grid", gap: 16 }}>
              ...
              <div>
                ...
                <div className="row gap-2 wrap">
                  {[
                    { k: "Payments", l: t("vSupport.catPayments", null, "Payments") },
                    { k: "Missions", l: t("vSupport.catMissions", null, "Missions") },
                    { k: "Account", l: t("vSupport.catAccount", null, "Account") },
                    { k: "Quality", l: t("vSupport.catQuality", null, "Quality") },
                    { k: "Other", l: t("vSupport.catOther", null, "Other") },
                  ].map(c => (
          ...
          {convos && convos.map(c => (
            <div key={c.id} style={{ alignSelf: c.fromAdmin ? "flex-start" : "flex-end", maxWidth: "85%", background: c.fromAdmin ? "var(--panel-inset)" : "var(--accent-weak)", padding: "12px 16px", borderRadius: 12 }}>
              <div className="row between gap-3" style={{ marginBottom: 6, fontSize: 12 }}>
                <b style={{ color: c.fromAdmin ? "var(--text)" : "var(--accent)" }}>{c.fromAdmin ? t("support.supportAgent", null, "Support Agent") : t("support.you", null, "You")}</b>
    ...
    <div className="page" style={{ maxWidth: 880, margin: "0 auto" }}>
      ...
      {tab === "help" && <div className="rise-3">
        ...
        <div className="card" style={{ padding: 0, overflow: "hidden" }}>
          {help.length === 0 ? <div style={{ padding: 32 }} className="faint">{t("support.noArticlesMatch", { q }, `No articles match "${q}".`)}</div> : help.slice(0, visibleHelpCount).map((h, i) => (
            <div key={i} style={{ borderTop: i ? "var(--hairline) solid var(--border)" : "none" }}>
              <button onClick={() => setOpen(open === i ? null : i)} style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", textAlign: "left", padding: "16px var(--pad-card)", background: "none", border: "none", cursor: "pointer" }}>
                ...
                <b style={{ flex: 1, fontSize: 14.5 }}>{helpArticleField(t, h._idx, "q", h.q)}</b>
              ...
              {open === i && <p className="muted" style={{ margin: 0, padding: "0 var(--pad-card) 18px 64px", fontSize: 14, lineHeight: 1.6 }}>{helpArticleField(t, h._idx, "a", h.a)}</p>}
            </div>
          ))}
          {visibleHelpCount < help.length && (
            <div style={{ textAlign: "center", padding: 16, borderTop: "var(--hairline) solid var(--border)" }}>
              <button className="btn btn-ghost" onClick={() => setVisibleHelpCount(c => c + 20)}>{t("actions.loadMoreArticles", null, "Load more articles")}</button>
            </div>
          )}
        </div>
        <div className="card" style={{ padding: "var(--pad-card)", marginTop: 16, display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ flex: 1 }}><b style={{ fontSize: 14.5 }}>{t("support.stillStuck", null, "Still stuck?")}</b><div className="faint" style={{ fontSize: 13 }}>{t("support.stillStuckDesc", null, "Raise a ticket and we'll reply within a few hours.")}</div></div>
      ...
      {tab === "tickets" && <div className="rise-3">
        {data.tickets.length === 0 ? <div className="card"><VEmpty icon="life" title={t("support.noTicketsYet", null, "No tickets yet")} body={t("support.noTicketsBody", null, "When you raise a support ticket it'll appear here.")} cta={<button className="btn btn-primary" onClick={() => setRaising(true)}>{t("support.raiseTicket", null, "Raise a ticket")}</button>} /></div> : (
          <div className="card" style={{ padding: 0, overflow: "hidden" }}>
            {data.tickets.slice(0, visibleTicketsCount).map((tObj, i) => {
              const st = getTicketStatus(t)[tObj.status] || getTicketStatus(t).open;
              return (
      ...
      {raising && <RaiseTicket onClose={() => setRaising(false)} onCreated={(newTicket) => setData(d => ({ ...d, tickets: [{ ...newTicket, updated: t("vSupport.justNow", null, "Just now") }, ...d.tickets] }))} />}
