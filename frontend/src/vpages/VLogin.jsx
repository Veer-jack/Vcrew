...
export default function VLogin() {
  ...
  const adapter = {
    ...
    sendSignupCode: (email, name) => vapi.sendSignupCode(email, name),
    login: (email, password) => vapi.login(email, password).then(({ token, validator }) => { setVToken(token); setValidator(validator); }),
    signup: ({ name, org, email, password, code }) => vapi.signup({ name, expertise: org, email, password, code, lang }).then(({ token, validator }) => { setVToken(token); setValidator(validator); }),
